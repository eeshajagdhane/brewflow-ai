...
def _extract_preferences_from_text(text: str, base: Optional[dict] = None) -> dict:
    ...
    if "flavor" not in base:
        for flavor in ("sweet", "fruity", "chocolate", "caramel", "vanilla", "matcha",
                       "mint", "berry", "mocha", "cinnamon", "pumpkin", "lavender"):
...
def run_manager_readiness(store_id: str, date: str, hour: int) -> dict:
    ...
    steps.append(make_step("detect_rush_period", "mcp_tool", rush_result["status"], rush_result["data"],
                            evidence=rush_result.get("evidence", []),
                            warnings=rush_result.get("warnings", [])))
    audit_log.append(make_audit_entry("call", "detect_rush_period", rush_result["status"],
    ...
    steps.append(make_step("calculate_staffing_gap", "mcp_tool", staffing_result["status"], staffing_result["data"],
                            evidence=staffing_result.get("evidence", []),
                            warnings=staffing_result.get("warnings", [])))
    audit_log.append(make_audit_entry("call", "calculate_staffing_gap", staffing_result["status"],
    ...
    steps.append(make_step("check_inventory_status", "mcp_tool", inv_result["status"], inv_result["data"],
                            evidence=inv_result.get("evidence", []),
                            warnings=inv_result.get("warnings", [])))
    audit_log.append(make_audit_entry("call", "check_inventory_status", inv_result["status"],
    ...
    steps.append(make_step("retrieve_internal_knowledge", "rag", "success",
                            {"docs_retrieved": len(rag_results)},
                            evidence=[{"source": r["filename"]} for r in rag_results]))
    audit_log.append(make_audit_entry("call", "retrieve_internal_knowledge", "success",
                                      f"Retrieved {len(rag_results)} policy docs."))

    # Step 5: Manager daily briefing
    briefing_result = run_manager_daily_briefing(store_id, date, hour)
    steps.append(make_step("manager_daily_briefing", "automation", briefing_result["status"],
                            briefing_result["data"],
                            evidence=briefing_result.get("evidence", []),
                            warnings=briefing_result.get("warnings", [])))
    audit_log.append(make_audit_entry("call", "manager_daily_briefing", briefing_result["status"],
    ...
    # Evidence package
    tool_outputs = [
        {"tool": "detect_rush_period", "status": rush_result["status"], "summary": str(rush_result["data"].get("demand_level", ""))},
        {"tool": "calculate_staffing_gap", "status": staffing_result["status"], "summary": f"gap={staffing_result['data'].get('staffing_gap', 0)}"},
        {"tool": "check_inventory_status", "status": inv_result["status"], "summary": f"available={inv_result['data'].get('availability_flag', True)}"},
        {"tool": "manager_daily_briefing", "status": briefing_result["status"], "summary": str(briefing_result["data"].get("readiness_summary", {}))},
    ]
...
def run_barista_recommendation(
    ...
    steps.append(make_step("search_menu_by_preferences", "mcp_tool", search_result["status"],
                            search_result["data"],
                            warnings=search_result.get("warnings", [])))
    audit_log.append(make_audit_entry("call", "search_menu_by_preferences", search_result["status"],
    ...
    tool_outputs = [
        ...
        {"tool": "search_menu_by_preferences", "status": search_result["status"],
         "summary": f"{len(candidates)} candidates"},
    ]
    if inv_result:
        tool_outputs.append({
            "tool": "check_inventory_status", "status": inv_result["status"],
    ...
    overall_status = "needs_review" if "needs_review" in statuses else ("warning" if "warning" in statuses else "success")
...
def run_order_builder(store_id: str, date: str, raw_order: str) -> dict:
    ...
    steps.append(make_step("build_and_validate_order", "mcp_tool", order_result["status"],
                            order_result["data"],
                            warnings=order_result.get("warnings", [])))
    audit_log.append(make_audit_entry("call", "build_and_validate_order", order_result["status"],
    ...
    steps.append(make_step("retrieve_internal_knowledge", "rag", "success",
                            ...
                            evidence=[{"source": r["filename"]} for r in rag_results]))
    audit_log.append(make_audit_entry("call", "retrieve_internal_knowledge", "success",
                                      f"Retrieved {len(rag_results)} confirmation policy docs."))

    all_warnings = list(dict.fromkeys(all_warnings))

    tool_outputs = [{"tool": "build_and_validate_order", "status": order_result["status"],
...
def run_full_workflow(
    ...
    evidence_package = build_evidence_package(
        merged_tool_outputs, merged_rag_results,
        assumptions=merged_assumptions, warnings=merged_warnings
    )
    ...
    overall_status = (
        "needs_review" if "needs_review" in phase_statuses
        else "warning" if "warning" in phase_statuses
        else "success"
    )
